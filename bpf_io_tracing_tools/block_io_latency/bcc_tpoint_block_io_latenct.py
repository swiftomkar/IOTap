...
b = BPF(text=bpf_text)
if args.queued:
    b.attach_tracepoint(tp="block:block_rq_issue", fn_name="trace_req_start")
else:
    #if BPF.get_kprobe_functions(b'blk_start_request'):
    #    b.attach_kprobe(event="blk_start_request", fn_name="trace_req_start")
    # b.attach_kprobe(event="blk_mq_start_request", fn_name="trace_req_start")
    b.attach_tracepoint(tp="block:block_rq_issue", fn_name="trace_req_start")
b.attach_tracepoint(tp="block:block_rq_complete", fn_name="trace_req_done")

print("bcc_tpoint_block_io_latency: tracing block IO latency in BCC using tracepoints")
...
while (1):
    ...
    if args.json:
        ...
        #    for k, v in self.items():
        #        if k.value != 0 and k.value <= max_nonzero_idx + 1:
        #            index = k.value  # math.pow(2, k.value)
        #            hist_dict[int(index)] = v.value
        #    return hist_dict
        #print(dist.get_hist_as_dict())
